...
template<class EOT>
void EORandomBreed<EOT>::operator() ( EOPop< EOT >& _ptVeo, EOPop< EOT >& _newPop )  { 
    ...
    if ( vecOp.size()  == 0 ) {
      cout << "\n ERROR:  No operators have been defined"<<endl;
      exit(0);
    }
    for ( i = 0; i < select; i ++ ) {
		// Create an alias of a random input EO with copy ctor
		EOT newEO = _ptVeo[ rand() % select ];
		...
		if (thisOp->readArity() == unary ) {
			...
			unsigned chosenMatch = rand() % select;
...
public:
  ...
  * all population randomly */
  virtual void operator() ( EOPop< EOT >& _ptVeo )  { 
	  ...
	  unsigned select= _ptVeo.size(); // New population same size than old	  
	  sort( EORandomBreed<EOT>::vecOp.begin(), EORandomBreed<EOT>::vecOp.end(), SortEOpPt<EOT>() );
	  
	  unsigned i;
	  float totalPriority = 0;
	  for ( i = 0; i < EORandomBreed<EOT>::vecOp.size(); i ++ ) {
		  totalPriority += EORandomBreed<EOT>::vecOp[i]->Priority();
	  }
	  ...
	  for ( i = 0; i < select; i ++ ) {
		  ...
		  typename vecOpT::const_iterator j;
		  float accumulated = 0;
		  for ( j = EORandomBreed<EOT>::vecOp.begin(); 
		  ( j != EORandomBreed<EOT>::vecOp.end() ) && ( accumulated < randomDraw); 
		  j ++ ) {
			  accumulated+= (*j)->Priority(); // the priority
		  }
		  
		  if ( j != EORandomBreed<EOT>::vecOp.begin() ) 
			  j--;			// previous one
		  EOOp<EOT >* thisOp = *j;
